...
const DEFAULT_IMAGES = Array.from({ length: 27 }, (_, i) => ({
  src: `gallery/${i + 1}.webp`,
  alt: `Gallery image ${i + 1}`
}));

// Memoized Image Tile Component for performance
const ImageTile = memo(({ item, index, onTileClick, onTilePointerUp }) => (
  <div
    key={`${item.x},${item.y},${index}`}
    className="item"
    data-src={getAsset(item.src)}
  ...
  >
    ...
    >
      <img  src={getAsset(item.src)}
       draggable={false} alt={item.alt} loading="lazy" decoding="async" />
    </div>
  </div>
));
...
// Then pass 'myLocalImages' to your DomeGallery component.
...
const DomeGallery = memo(({
  ...
  const animate = () => {
    // Only rotate if the user is NOT dragging or viewing a photo
    const isInteracting = 
      draggingRef.current || 
      openingRef.current || 
    ...
    if (!isInteracting) {
      // 0.15 is the speed. Increase it for faster rotation
      rotationRef.current.y = wrapAngleSigned(rotationRef.current.y + 0.15);
      
      // Apply the new rotation to the sphere - throttle to 60fps
  ...
  const onTileClick = useCallback(
    e => {
      ...
      requestAnimationFrame(() => openItemFromElement(el));
    },
    [openItemFromElement]
  );
  ...
  return (
    ...
    >
      ...
      style={{ 
          backgroundImage: `url(${getAsset('/gallery/bg.webp')})`,
